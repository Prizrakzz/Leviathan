...
def steps(run_id: str) -> list[tuple[str, list[str]]]:
    ...
    return [
        ...
        ("S2  MOVE FOUR JOBDEFS -- copy the live revision verbatim; the envelope must survive.", [
            ...
            "# ---- AND WHEN A LANE *DOES* MOVE A TERRAFORM RESOURCE ------------------------",
            "# NONE of the four jobdefs above is a terraform resource, so THIS lane plans and",
            "# applies no terraform at all -- S9 says the same thing about the schedule. The",
            "# terraform-managed jobdefs (module.batch.aws_batch_job_definition.*, count-gated)",
            "# belong to other lanes, and there the apply is gated on the SAVED PLAN'S JSON,",
            "# between the plan and the apply, never on an eyeballed diff:",
            "#   terraform -chdir=infra/terraform/envs/dev plan -target=<addr> "
            "-out=lane.tfplan",
            ...
            "--plan-json infra/terraform/envs/dev/tfplan_lane.json "
            "--address <addr> --expect-changed container_properties.image "
            "--expect-unchanged-envelope",
            "#   terraform -chdir=infra/terraform/envs/dev apply lane.tfplan   # ONLY on exit 0",
            "# BOTH ARTIFACTS ARE NAMED SO .gitignore ALREADY COVERS THEM, and this is the same",
            "# shape the tool's own docstring and the pink runbook print -- one recipe, pinned by",
            "# tests/unit/test_tf_single_resource_gate.py, which reddens on any divergence.",
            "# THE RENDERING goes inside the env directory and is named tfplan_*: .gitignore:72",
            "# (infra/terraform/**/tfplan*) covers it there, while a tf_plan.json in the repo",
            "# root is NOT ignored. THE BINARY SAVED PLAN is named <stem>.tfplan: terraform",
            "# resolves -out and show's argument INSIDE -chdir's directory, so lane.tfplan lands",
            "# at infra/terraform/envs/dev/lane.tfplan and .gitignore:73",
            "# (infra/terraform/**/*.tfplan) covers it -- where the tf.plan spelling this recipe",
            "# used to carry landed at infra/terraform/envs/dev/tf.plan, which matches NEITHER",
            "# rule. MEASURED 2026-09-07 with terraform v1.15.2 and `git check-ignore -v` on",
            "# every spelling.",
            "# BOTH HALVES carry the account id, every ARN and the INLINE IAM POLICY DOCUMENTS",
            "# of the roles in the plan -- the JSON is rendered FROM the binary, it is the same",
            "# payload; .gitignore:71 already calls saved plans sensitive, this repo is PUBLIC,",
            "# and co-tenant agents race the git index. Note that -chdir does NOT apply to the",
            "# redirect: Out-File writes relative to YOUR cwd, which is why the JSON path is",
            "# spelled in full and the .tfplan is not.",
            "# MEASURED 2026-09-06: a hand-written gate refused a CORRECT futures_eod_silver",
            "# repin three times -- the live address carries [0], arn and revision are computed",
            "# after apply, and eight container keys read as moved raw (the provider's own",
            "# empties plus environment order) while ONLY image had moved.",
            "# ITS --expect-unchanged-envelope CLAUSE IS THE SAME KIND OF ASSERTION as the",
            "# --expect-memory 12288 above, NOT the same number: the 12,288 MiB post-OOM bump",
            "# lives on these four NON-terraform jobdefs and can never reach that clause (grep",
            "# -c 12288 infra/terraform/modules/batch/main.tf = 0; the terraform-managed jobdefs",
            "# carry 512/1024/2048/4096/8192/16384, and the only envelope the gate has ever read",
            "# is VCPU 1 / MEMORY 4096 on the futures plan). Both refuse the same failure: a",
            "# descriptor re-authored from constants reverting an envelope silently.",
        ] + [
