...
def transform_fgis_bronze_to_silver(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop any rows where marketing_year is NA (guard against upstream bronze issues).
    na_my = unique["marketing_year"].isna()
    if na_my.any():
        logger.warning(
            "FGIS silver: dropping %d unique triple(s) with null marketing_year",
            na_my.sum(),
        )
        unique = unique[~na_my].reset_index(drop=True)
        df = df[df["marketing_year"].notna()].reset_index(drop=True)
