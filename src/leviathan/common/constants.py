...
MIN_RAW_FILE_SIZES: dict[str, int] = {
    ...
    "minagro_grain_exports": 3_000,                   # 3 KB -- the landed object is the rendered <main> outerHTML: 6,842 B for the 2026-08-14 capture (10 commodity rows + the breadcrumb/headline chrome). A Cloudflare managed-challenge interstitial is a few KB and CAN clear this floor, which is exactly why size is NOT the defence here: transforms.raw_to_bronze.minagro_grain_exports.looks_like_the_export_table refuses the capture on MARKERS before any byte is landed, and this floor is only the truncated-render backstop
}
