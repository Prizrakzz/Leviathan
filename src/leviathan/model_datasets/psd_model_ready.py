...
def _is_snapshot_dynamic_feature(feature: str) -> bool:
    """Return true for features generated by the snapshot-time dynamic builders."""
    name = str(feature)
    if name.startswith("wasde_"):
        return True
    return (
        name.startswith("psd_")
        and name not in PSD_SNAPSHOT_DYNAMIC_ID_COLUMNS
        and any(name.endswith(suffix) for suffix in PSD_VINTAGE_FEATURE_SUFFIXES)
    )
