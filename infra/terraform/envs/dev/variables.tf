...
variable "silver_lane_alarms" {
  ...
  description = "failure_mode -> the text/corpus lane alarm (corpus fold liveness / wrote-nothing, WASDE text errors / infra errors), generated by jobs/observability/silver_alarms.py --emit-tfvars and admitted only when silver_published_metrics.json lists the metric stream. Never hand-set. Empty default = no lane alarms."
  default     = {}
}
