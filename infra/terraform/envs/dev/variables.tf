...
variable "worker_fleet_image_digest" {
  ...
  default     = "sha256:87948f1e1051b48883e5dd14c54bba2e2ee1cbf98935be8dceefe5969ecf769a" # 2026-08-27 FERTILIZER/ENERGY fleet bump (commit bcf570ce): the pink_sheet_input_costs basket split -- 69 cost-push legs wired onto 7 price-context rows (brent/urea/gas US+EU/npk/dap/potash z twins), 14 nodes retagged planned per review wf_b69ce788-4e1, register UNCHANGED at 20 (rubber refuted on sign-identity). Census truth on this vintage: 757 legs / 506 FIRES / 251 DECLINES / 0 DARK (the 8th consecutive exact prediction; enumeration half proven locally pre-measurement). The gate MUST ride this vintage: its own lints now refuse an omitted `agg` on a price-context row (the fail-open default disagreement closed fail-closed in cascade.py), and a scheduled [Reconcile] on the prior vintage would re-bank the pre-split 688 census over the 757 baselines -- the exact stale-census-bank class the 19146ded bump closed. PRIOR: sha256:19146ded... (2026-08-26 EVE, commit f99a03d6, census 688/437 -- the stale-census-bank incident close).
...
variable "silver_gate_image_digest" {
  ...
  default     = "sha256:87948f1e1051b48883e5dd14c54bba2e2ee1cbf98935be8dceefe5969ecf769a" # 2026-08-27 (commit bcf570ce): the gate MUST ride the vintage whose lints and configs it enforces -- this one carries the fertilizer/energy basket split (7 price-context rows the gate's own check_price_register validates, incl. the fail-closed omitted-agg term the prior vintage lacked) and the 757-leg census map whose [Reconcile] step banks the rolling baselines; a gate on the prior vintage would re-bank the pre-split 688 census over the 757 baselines (the stale-census-bank class). PRIOR: sha256:19146ded... (2026-08-26 EVE, commit f99a03d6 -- the R4 amendment + keying-knob + country_name_ref class-fix vintage).
