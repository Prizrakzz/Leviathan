...
# ---------------------------------------------------------------------------
# P6, 2026-09-22 -- THE DATA AXIS. Every alarm above this block reads S3 WRITE RECENCY, so a
# producer that re-writes a byte-identical object on its fire cadence is GREEN forever while its
# content is dead. MEASURED by a read-only poll of this estate on 2026-09-22:
# silver_sagis_weekly_exports FreshnessLagRatio 0.211 (green) beside DataDateAgeRatio 46.263;
# silver_unica_corn_ethanol 0.430 beside 16.643. Eleven tables breach on the data axis and seven of
# them are green on every write-axis alarm in this account.
#
# THRESHOLD 1.0 IS THE TABLE'S OWN DECLARED CEILING, not a number invented in telemetry -- the same
# argument that justified FreshnessLagRatio's 1.0 in D-PR-14, which is why this does not have to
# wait for the "week of measured data" silver_rebuild_gate.py:283 asked for before alarming the
# DAY metric.
#
# treat_missing_data = "notBreaching" HERE ONLY, and it is not fail-open: the ABSENCE of a reading
# is alarmed once and precisely by data_date_unread below, a FORWARD-stamped axis by
# data_date_ahead, and a poller that did not run at all by freshness_targets_polled (the one
# breaching alarm of the four). Making these 31 "breaching" would put every one into ALARM on the
# apply that CREATES them, before the emitter image ships -- this module's own header names that
# hazard for the pre-emit families; this would be it times thirty-one.
#
# DAY-ONE EXPECTATION, stated so the first page is recognised as the alarm working: 11 of the 31 go
# ALARM on the first poll cycle after the emitter ships, and every one of the eleven is a census
# finding with a named owner (sagis_weekly_exports 46.26x, unica_corn_ethanol 16.64x,
# unica_season_history 11.10x, unica_monthly_sales 7.22x, board_crush 5.50x, futures_spreads 5.33x,
# fnc_port_type 4.56x, fnc_monthly 3.87x, minagro 2.79x, fgis 1.64x, nass_citrus 1.09x).
# ---------------------------------------------------------------------------
resource "aws_cloudwatch_metric_alarm" "data_date_age_breach" {
  for_each = var.silver_data_date_slas

  alarm_name          = "${local.name_prefix}-data-date-age-breach-${replace(each.key, "_", "-")}"
  alarm_description   = "Table '${each.key}' (${each.value.family}) holds DATA older than its own declared ceiling (${each.value.ceiling_days}d; ${each.value.basis}). CONTENT axis, not write axis: a producer that re-writes an identical object every fire reads fresh on FreshnessLagDays while this alarm is the one that speaks. Emitted by jobs/observability/freshness_poller_task.py (DataDateAgeRatio, dim Table). Runbook: R4_incident_runbooks.md#freshness-sla-breach."
  namespace           = var.silver_metric_namespace
  metric_name         = "DataDateAgeRatio"
  dimensions          = { Table = each.key }
  statistic           = "Maximum"
  period              = 86400
  evaluation_periods  = 1
  comparison_operator = "GreaterThanThreshold"
  threshold           = each.value.ratio_threshold
  treat_missing_data  = "notBreaching"
  alarm_actions       = local.alarm_actions
  tags                = { Project = var.project_name, Environment = var.environment, ManagedBy = "terraform" }

  # THE INTERLOCK, and the reason var.silver_data_date_static is an input to this module at all.
  # The static EXCLUSION happens in the generator (freshness.data_date_alarm_targets subtracts
  # STATIC_DATA_TARGETS before the map is written), so the two maps are disjoint by construction
  # TODAY. This makes that construction FAIL THE PLAN instead of being a comment: if a future
  # generator change, a merge, or a hand edit of the held tfvars ever put a declared closed archive
  # into the alarm map, terraform stops here and names the table, rather than applying a
  # permanently-red alarm on an archive nobody decided to watch.
  lifecycle {
    precondition {
      condition     = !contains(keys(var.silver_data_date_static), each.key)
      error_message = "Table ${each.key} is declared a CLOSED ARCHIVE in silver_data_date_static and must not carry a data_date_age_breach alarm. Both maps are generated by jobs/observability/silver_alarms.py --emit-tfvars; re-run the generator rather than editing silver_observability.auto.tfvars.json (D-EI-12 hold)."
    }
  }
}

# ---------------------------------------------------------------------------
# THE BLINDNESS ALARM. One alarm for the whole class -- an unreadable parquet footer, a declared
# column absent from the schema, an unparseable partition value, a missing pyarrow, an AccessDenied
# on the footer GET. This is what lets every per-table alarm above be notBreaching with no door
# left open. Reason=unreadable ONLY: a forward-stamped axis has its own alarm below, an empty
# canonical prefix is already the write axis's loudest word (and two tables in this estate are
# deliberately empty under the four-checkmark law), a declared closed archive is
# var.silver_data_date_static, an undeclared axis is a repo fact pinned by name in
# tests/unit/silver/test_freshness_poller.py, and "disabled" is an operator holding the documented
# --no-data-date rollback lever down.
# ---------------------------------------------------------------------------
resource "aws_cloudwatch_metric_alarm" "data_date_unread" {
  count = length(var.silver_data_date_slas) > 0 ? 1 : 0

  alarm_name          = "${local.name_prefix}-data-date-unread"
  alarm_description   = "At least one poll target whose DATA-date axis is DECLARED could not be read this cycle (unreadable parquet footer, declared column absent from the schema, unparseable partition value, or pyarrow missing from the image). While this is in ALARM the per-table data_date_age_breach alarms are blind for that target, which is exactly why they are notBreaching. ${length(var.silver_data_date_static)} declared closed archive(s) are excluded from the per-table alarms by design and are not counted here. Emitted by jobs/observability/freshness_poller_task.py (DataDateUnread, dim Reason=unreadable)."
  namespace           = var.silver_metric_namespace
  metric_name         = "DataDateUnread"
  dimensions          = { Reason = "unreadable" }
  statistic           = "Maximum"
  period              = 86400
  evaluation_periods  = 1
  comparison_operator = "GreaterThanThreshold"
...
# ---------------------------------------------------------------------------
# THE FORWARD-STAMP ALARM (round 2, adversarial review M3). A target whose DECLARED knowledge axis
# reads into the future publishes NO DataDateAgeRatio datapoint -- correctly, a future date is not
# a freshness figure -- and its per-table alarm above is notBreaching, so without this resource the
# table leaves the content watch in complete silence. MEASURED 2026-09-22: silver_nass_annual's
# declared axis (release_date) holds 2027-02-01, 132 days ahead.
#
# IT ALARMS RATHER THAN MERELY COUNTING because a knowledge row stamped in the future is one guard
# away from a point-in-time LEAK -- this estate's whole evidence contract is that no row whose
# knowledge date is after the question's as-of may be served.
#
# RED ON ARRIVAL, ON PURPOSE. The threshold is 0 and the known set is NOT excused: a threshold of
# "however many read ahead today" would be round 1's silence wearing a number, and it would
# re-hide the next forward-stamped contract the moment one was added.
# leviathan.silver.freshness.DATA_DATE_AHEAD_KNOWN names today's one table, its measured lead and
# its remedy (declare a data-PERIOD column on the NASS annual contract instead of a SCHEDULED
# release date). This alarm clears the day that declaration lands.
# ---------------------------------------------------------------------------
resource "aws_cloudwatch_metric_alarm" "data_date_ahead" {
  count = length(var.silver_data_date_slas) > 0 ? 1 : 0

  alarm_name          = "${local.name_prefix}-data-date-ahead"
  alarm_description   = "At least one poll target's DECLARED knowledge axis points INTO THE FUTURE past the one-day clock-skew tolerance, so it publishes no age datapoint and its notBreaching per-table alarm reads OK while nothing watches its content -- and a forward-stamped knowledge row is one guard away from a point-in-time leak. EXPECTED RED AT CREATION: silver_nass_annual reads release_date=2027-02-01 (132d ahead, measured 2026-09-22). Remedy: declare a data-PERIOD column on that contract, or fix the producer that stamped a future date; the declared set and each remedy live in leviathan.silver.freshness.DATA_DATE_AHEAD_KNOWN. Emitted by jobs/observability/freshness_poller_task.py (DataDateUnread, dim Reason=ahead)."
  namespace           = var.silver_metric_namespace
  metric_name         = "DataDateUnread"
  dimensions          = { Reason = "ahead" }
  statistic           = "Maximum"
  period              = 86400
  evaluation_periods  = 1
  comparison_operator = "GreaterThanThreshold"
...
# ---------------------------------------------------------------------------
# P7 -- THE POLLER'S OWN CENSUS. It has printed its target count on line 1 of every run since it
# was built and nothing has ever read it. MEASURED 2026-09-21: it printed 46 while the repo
# registry enumerated 53 -- a baked image five weeks behind HEAD, four contracts unmeasured and a
# RETIRED surface still polled at ratio 2.34, invisible for five weeks.
#
# treat_missing_data = "breaching": a poller that cannot say how many targets it polled did not
# run. This is the ONE new alarm that pages on absence, and it is what makes the other three safe.
# DO NOT APPLY THIS ONE BEFORE THE REPIN -- see the ORDER in the lane 7 fix report. Until the image
# carrying the emitter ships it is TRUE and therefore red.
# ---------------------------------------------------------------------------
resource "aws_cloudwatch_metric_alarm" "freshness_targets_polled" {
  count = var.silver_expected_poll_targets > 0 ? 1 : 0

  alarm_name          = "${local.name_prefix}-freshness-targets-polled"
  alarm_description   = "The freshness poller polled FEWER than the ${var.silver_expected_poll_targets} targets this repo's registry declares -- i.e. the image it runs from carries an OLDER configs/silver/tables than HEAD, and every contract added since that image is unmeasured and unalarmed. Remedy: repin the jobdef the leviathan-dev-freshness-poller schedule targets (leviathan-dev-raw-ingest-runner) onto a CURRENT worker image with scripts/ops/repin_jobdef_digest.py."
  namespace           = var.silver_metric_namespace
  metric_name         = "FreshnessTargetsPolled"
  statistic           = "Minimum"
  period              = 86400
  evaluation_periods  = 1
  comparison_operator = "LessThanThreshold"
  threshold           = var.silver_expected_poll_targets
  treat_missing_data  = "breaching"
  alarm_actions       = local.alarm_actions
  tags                = { Project = var.project_name, Environment = var.environment, ManagedBy = "terraform" }
}
