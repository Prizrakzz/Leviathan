-- Immutable long-format feature spine versions for MLflow dataset pinning.
-- Same fixed schema as gold_feature_spine, with dataset_version as a partition.
CREATE EXTERNAL TABLE IF NOT EXISTS gold_feature_spine_versions (
    country     string,
    crop_year   int,
    feature     string,
    value       double,
...
STORED AS PARQUET
LOCATION 's3://leviathan-dev-shahem-001/gold/feature_spine_versions/'
TBLPROPERTIES (
    ...
    'storage.location.template' = 's3://leviathan-dev-shahem-001/gold/feature_spine_versions/dataset_version=${dataset_version}/commodity=${commodity}/'
);
