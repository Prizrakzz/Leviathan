...
| Futures price series — Quandl CHRIS (C1/C2/C3 calendar spreads) | ❌ **WAF blocked** — Nasdaq Data Link's Cloudflare WAF blocks the entire `/api/v3/datasets/` path for all programmatic access (confirmed from home, Docker, and AWS Lambda us-east-1). Appears to require paid subscription or browser session. **Pipeline code is fully written and committed** (`quandl_chris_task.py`); will run immediately once access is resolved. Options: (1) Nasdaq Data Link paid plan ~$30/mo; (2) Barchart API alternative; (3) accumulate yfinance forward curve snapshots going forward. Deferred to P9. |
