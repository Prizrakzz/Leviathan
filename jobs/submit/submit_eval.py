...
def build_command(*, queries: str, model: str, judge: bool, judge_model: str, k: int) -> list[str]:
    """The container command (the image ENTRYPOINT is `python`, so this is the arg list to it)."""
    cmd = ["-m", "leviathan.graphrag.eval", "--run", "--model", model, "--k", str(k), "--queries", queries]
    if judge:
...
def main() -> None:
    ...
    command = build_command(queries=args.queries, model=args.model, judge=args.judge,
    ...
    write_run_record(Path("data/batch_runs") / f"eval_{run_id}.json",
                     {"run_id": run_id, "job_name": job_name, "job_id": resp["jobId"],
                      "queries": args.queries, "model": args.model, "judge": args.judge})
