"""Submit CONAB coffee bronze -> silver as an AWS Batch task."""
...
logger = get_logger("submit_batch_conab_coffee_silver")
...
def save_run_record(submitted: list[dict], params: dict[str, str]) -> None:
    ...
    write_run_record(
        Path("data/batch_runs") / f"conab_coffee_silver_{run_id}.json",
        payload,
    )
...
def main() -> None:
    ...
    job_definition = f"{project}-{env}-conab-coffee-silver"

    parser = argparse.ArgumentParser(description="Submit CONAB coffee bronze->silver Batch task.")
    ...
    logger.info(
        "Submitting CONAB coffee silver task queue=%s definition=%s dry_run=%s",
        batch_queue,
        job_definition,
        args.dry_run,
    )
    submitted = submit_batch_jobs(
        ...
        build_job_name=lambda _: "conab-coffee-silver",
        aws_region=aws_region,
    ...
    logger.info("Submitted CONAB coffee silver task: %s", submitted[0].get("job_id"))
