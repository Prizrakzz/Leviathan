"""Submit CPC Soil Moisture bronze → silver as AWS Batch Fargate tasks.
...
Usage:
    python jobs/submit/submit_batch_b2s_cpc_soil.py
    python jobs/submit/submit_batch_b2s_cpc_soil.py --commodities arabica_coffee,corn_cbot
    python jobs/submit/submit_batch_b2s_cpc_soil.py --dry-run
"""
...
logger = get_logger("submit_batch_b2s_cpc_soil")
...
def submit_tasks(
    ...
    return submit_batch_jobs(
        ...
        build_job_name=lambda t: f"cpc-soil-b2s-{t['commodity']}".replace("_", "-"),
...
def save_run_record(submitted: list[dict], commodities: list[str]) -> None:
    ...
    write_run_record(Path("data/batch_runs") / f"cpc_soil_b2s_{run_id}.json", payload)
...
def main() -> None:
    ...
    b2s_job_def = f"{project}-{env}-cpc-soil-bronze-to-silver"

    parser = argparse.ArgumentParser(
        description="Submit CPC Soil Moisture bronze→silver as AWS Batch Fargate tasks."
    )
    parser.add_argument(
        "--commodities",
        default="all",
