...
def _is_permanent_404(exc: Exception) -> bool:
    """True if *exc* is an HTTP 404 — a permanently-pruned historical file.
...
def _exit_reason(uploaded: int, skipped: int, errors: int, missing: int) -> str | None:
    ...
    """
    if errors:
        return f"{errors} file(s) failed - see logs above."
    if uploaded == 0 and skipped == 0 and missing > 0:
        return f"No files uploaded and {missing} link(s) returned 404 - source may be dead."
    return None
