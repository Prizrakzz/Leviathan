"""AWS Batch entrypoint: ICCO cocoa bronze -> silver via the shadow-first publisher.

SILVER-F051 (half-orphan restore, Milestone R3). Reads the QBCS bronze corpus
(``bronze/production/source=icco_qbcs/**/part-000.parquet``) that ``fetch_icco_qbcs_summary.py``
feeds, reduces it to the canonical one-row-per-cocoa-year silver via
:func:`~leviathan.transforms.bronze_to_silver.icco_cocoa.build_icco_silver`, and PUBLISHES silver
through the common SILVER-F015 shadow-first publisher (INV-2 explicit schema; V001 value gate).
...
(execution gated to BF-W3). This is the bounded backfill entrypoint (``--publish-mode`` is the gate).

Usage
-----
    python jobs/batch/icco_cocoa_task.py                        # dry-run (writes nothing)
...
from leviathan.transforms.bronze_to_silver.icco_cocoa import build_icco_silver

logger = get_logger("icco_cocoa_task")

_TABLE = "silver_icco_cocoa"
_BRONZE_PREFIX = "bronze/production/source=icco_qbcs/"
...
def _read_bronze_corpus(bucket: str, prefix: str, aws_region: str) -> pd.DataFrame:
    """Read + concat every bronze parquet under ``prefix`` (read-only)."""
...
def main() -> None:
    ...
    load_env()
    parser = argparse.ArgumentParser(description="ICCO cocoa bronze -> silver via the shadow publisher")
    ...
    df_silver = build_icco_silver(df_bronze)
    ...
    plan = build_flat_publish(
        df=df_silver, contract=contract, canonical_key=silver_icco_cocoa_key(), auth=auth,
        s3_client=s3_client, job="icco_cocoa_task", run_id=args.run_id, code_sha=args.code_sha,
        manifest_store=manifest_store, min_rows=1,
    )
    manifest = plan.run()
    logger.info("ICCO publish complete: state=%s rows=%d mode=%s",
