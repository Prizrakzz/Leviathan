"""Audit WASDE snapshot anomaly-detector inputs.

The audit is read-only against S3. It loads existing model-ready snapshot
matrices, checks row grain, feature coverage, event balance, and prior-history
viability, then writes local audit artifacts.
"""
...
from leviathan.model_datasets.wasde_snapshot_anomaly_audit import (  # noqa: E402
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-independent-groups", type=int, default=100)
    parser.add_argument("--min-event-groups", type=int, default=20)
    parser.add_argument("--min-prior-observations", type=int, default=10)
    parser.add_argument(
        "--include-static-features",
        action="store_true",
        help="Audit static context features in addition to WASDE dynamic columns.",
    )
...
def main() -> None:
    ...
    outputs = {
        "phase0_audit": _write_json(output_dir / "phase0_audit.json", report),
        "feature_coverage": _write_parquet(
            output_dir / "phase0_feature_coverage.parquet",
            result.feature_coverage,
        ),
        "event_distribution": _write_parquet(
            output_dir / "phase0_event_distribution.parquet",
            result.event_distribution,
        ),
        "prior_history": _write_parquet(
            output_dir / "phase0_prior_history.parquet",
