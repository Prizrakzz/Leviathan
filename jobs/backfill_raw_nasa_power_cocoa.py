...
logger = get_logger("backfill_raw_nasa_power_cocoa")
...
def build_raw_filename(
    ...
    return f"nasa_power_cocoa_{country}_{region}_{start_date}_{end_date}.json"
...
def build_local_raw_path(
    ...
    return Path("data/raw/weather/nasa_power/cocoa") / country / region / filename


def build_metadata_key(run_id: str, filename: str) -> str:
    return f"metadata/runs/source=nasa_power/commodity=cocoa/run_id={run_id}/{filename}"
...
def main() -> None:
    ...
    logger.info("Starting NASA POWER cocoa backfill run_id=%s", run_id)
    ...
    metadata_path = write_json_metadata(
        run_summary,
        Path("data/metadata/runs/nasa_power/cocoa") / f"run_{run_id}.json",
    )

    logger.info("Wrote run metadata: %s", metadata_path)

    if args.upload:
        metadata_key = build_metadata_key(run_id=run_id, filename=metadata_path.name)
        upload_file_to_s3(
